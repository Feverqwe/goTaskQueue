...
create and follow tasks; read log tails, incremental output, or just the current
PTY screen; rerun or stop tasks; and send input or resize messages to PTY tasks.
Completed, canceled, and failed tasks can also be cleaned up in bulk. A PTY
shell has the same effective access as the operating-system user running
GoTaskQueue. Treat the MCP token like an SSH credential, expose the endpoint
only over trusted HTTPS, and do not run GoTaskQueue as root.
